...
def validate_samples(samples, quiet: bool = False):
    ...
    if len(err_acc) > 0:
        console.print(
            Panel.fit(
                f"\n[dim]The following samples could not be found locally yet do not have an accession ID provdied for SRA search:\n\n{err_acc}\n\nPlease make sure that files either exists, accession ID is provided, or R1 and R2 path columns are correctly setup in sample_list.csv file.",
                title="Sample Accession Error",
                subtitle="File Not Found Locally",
            )
        )
        sys.exit(1)
    ...
    if len(notfound_acc) > 0:
        ...
        for i in range(0, len(notfound_acc), 500):
            ...
            try:
                ...
                console.print(
                    Panel.fit(
                        f'[dim]HTTP Error {err.code} has occured when running Entrez.efetch(). NOTE: An  HTTP Error 400 indicates that the server cannot or will not process the request due to a client-side error. This could be becuase NCBI has flagged your IP address due to too many request. To resolve this issue, create an NCBI API key and pass it on to the command line via the "--NCBI-API-key" command line option. Visit https://support.nlm.nih.gov/knowledgebase/article/KA-05317/en-us for more information on how to obtain a key!',
                        title=f"HTTP Error {err.code}",
                        subtitle="NCBI Entrez efetch Error",
                    )
                )
                sys.exit(1)
        ...
        if len(missing_acc) != 0:
            console.print(
                Panel.fit(
                    f"[dim]Samples with the following  accessions could not be found locally or on NCBI's Entrez Direct search via the SRA:\n\n{missing_acc}\n\nPlease make sure that:\nA) The Accession can be found by efetch in NCBI's Entrez Direct\nB) The accession is a valid run\n\nIf you intend to use locally stored fastq files, follow the format suggested above\nWARNING: The error could be because NCBI server's are busy and cannot do a large search for > 1000 samples. Try again later!",
                    title="Sample Accession Error",
                    subtitle="SRA Accession Not Parsed",
                )
            )
            sys.exit(1)
...
def parse_sample_list(f, datadir, outdir, email, api_key, nowstr, quiet: bool = False):
    ...
    try:
        if os.path.isfile(f) and f.endswith(".csv"):
            df = pd.read_csv(
                f, comment="#", header=0, sep=",", index_col=False, dtype=str
            )
        else:
            console.print(
                Panel.fit(
                    f'[dim]Sample List input "{f}" is not a comma separated table (.csv format).',
                    title="Sample List Error",
                    subtitle="Sample List not CSV.",
                )
            )
            sys.exit(1)
        ...
        console.print(
            Panel.fit(
                f'Sample List "{f}" could not be read by pd.read_csv() or does not exist',
                title="Sample List Error",
                subtitle="Sample List not CSV.",
            )
        )
    ...
    for i, row in df.iterrows():
        if pd.isna(row["sample_id"]) and not pd.isna(row["accession"]):
            ...
            raise ValueError(
                "Column with both empty sample_id and SRA accession, please provide at least one"
            )

    # if there are no assemblies set, make it the same as sample_id (single sample assembly)
    df["assembly"] = [
        row["assembly"] if not pd.isna(row["assembly"]) else row["sample_id"]
    ...
    df["R1"] = [
        (
            row["R1"]
            if not pd.isna(row["R1"])
            else "{dir}{s}_1.fastq.gz".format(dir=datadir, s=row["sample_id"])
        )
        for _, row in df.iterrows()
    ]
    df["R2"] = [
        (
            ...
            else "{dir}{s}_2.fastq.gz".format(dir=datadir, s=row["sample_id"])
        )
        for _, row in df.iterrows()
    ]
    ...
    if df.index.duplicated().any():
        console.print(
            Panel.fit(
                "ValueError on df.set_index('sample_id'). Values in the sample_id column may be non-unique {}. Please check your Sample List file.".format(
                    df.index[df.index.duplicated()]
                ),
                title="Value Error",
                subtitle="Duplicate 'sample_id' Names",
            )
        )
        raise ValueError()
    ...
    if not (duplicaterow.empty and duplicateid.empty):
        ...
        console.print(
            Panel.fit(
                "Duplicate rows or SRA accessions found.\nPlease check your sample_list.csv file.\nWarning list:\n{}\n\nAt the moment having the same file in different assemblies is not supported, but we are working on it for future versions".format(
                    duplist
                ),
                title="Sample ID Error",
                subtitle="Duplicate Sample IDs",
            )
        )
        sys.exit(1)
