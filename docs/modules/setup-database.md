...
vomix-snakemake handles all ad-hoc steps of viral metagenomics, including cumbersome database setups for each software. The `vomix setup-database` module includes the installation of all databases needed for full vomix-snakemake usage. However, when running any of the modules below, vomix-snakemake pre-determines each database needed for the module and automatically downloads it using the `--setup-database` flag. The size of each database and available free disk space is disclosed when running the dry run (`--dry-run` or `-n-`).
...
```bash
vomix setup-database -h
vomix setup-database --sdm conda --samplelist sample/sample_list.csv -j 16 --latency-wait 20
```
...
```bash
vomix setup-database -h
vomix setup-database --sdm conda --sdm apptainer --samplelist sample/sample_list.csv -j 16 --latency-wait 20
```
...
vomix setup-database --sdm conda --samplelist sample/sample_list.csv -j 16 --latency-wait 20 --executor cluster-generic --cluster-generic-submit-cmd "qsub -N {log} -l nodes=1:ppn={threads} -l mem={resources.mem_mb}m -l walltime=120:00:00 -M $EMAIL -q $QUEUE -o qsub.log -e qsub.log -m a"
```
...
EMAIL="your.email@example.com"
QUEUE="cluster_queue_name"
vomix setup-database --sdm conda --sdm apptainer --samplelist sample/sample_list.csv -j 16 --latency-wait 20 --executor cluster-generic --cluster-generic-submit-cmd "qsub -N {log} -l nodes=1:ppn={threads} -l mem={resources.mem_mb}m -l walltime=120:00:00 -M $EMAIL -q $QUEUE -o qsub.log -e qsub.log -m a"
```
...
```bash
vomix setup-database -h
vomix setup-database --sdm conda --fasta sample/contigs/contigs_simulated_viral_nonviral.fasta -j 16 --latency-wait 20
```
...
```bash
vomix setup-database -h
vomix setup-database --sdm conda --sdm apptainer --fasta sample/contigs/contigs_simulated_viral_nonviral.fasta -j 16 --latency-wait 20
```
...
EMAIL="your.email@example.com"
QUEUE="cluster_queue_name"
vomix setup-database --sdm conda --fasta sample/contigs/contigs_simulated_viral_nonviral.fasta -j 16 --latency-wait 20 --executor cluster-generic --cluster-generic-submit-cmd "qsub -N {log} -l nodes=1:ppn={threads} -l mem={resources.mem_mb}m -l walltime=120:00:00 -M $EMAIL -q $QUEUE -o qsub.log -e qsub.log -m a"
```
...
EMAIL="your.email@example.com"
QUEUE="cluster_queue_name"
vomix setup-database --sdm conda --sdm apptainer --fasta sample/contigs/contigs_simulated_viral_nonviral.fasta -j 16 --latency-wait 20 --executor cluster-generic --cluster-generic-submit-cmd "qsub -N {log} -l nodes=1:ppn={threads} -l mem={resources.mem_mb}m -l walltime=120:00:00 -M $EMAIL -q $QUEUE -o qsub.log -e qsub.log -m a"
```
...
```bash
vomix setup-database -h
vomix setup-database --sdm conda --fastadir sample/contigs/ -j 16 --latency-wait 20
```
...
```bash
vomix setup-database -h
vomix setup-database --sdm conda --sdm apptainer --fastadir sample/contigs/ -j 16 --latency-wait 20
```
...
EMAIL="your.email@example.com"
QUEUE="cluster_queue_name"
vomix setup-database --sdm conda --fastadir sample/contigs/ -j 16 --latency-wait 20 --executor cluster-generic --cluster-generic-submit-cmd "qsub -N {log} -l nodes=1:ppn={threads} -l mem={resources.mem_mb}m -l walltime=120:00:00 -M $EMAIL -q $QUEUE -o qsub.log -e qsub.log -m a"
```
...
EMAIL="your.email@example.com"
QUEUE="cluster_queue_name"
vomix setup-database --sdm conda --sdm apptainer --fastadir sample/contigs/ -j 16 --latency-wait 20 --executor cluster-generic --cluster-generic-submit-cmd "qsub -N {log} -l nodes=1:ppn={threads} -l mem={resources.mem_mb}m -l walltime=120:00:00 -M $EMAIL -q $QUEUE -o qsub.log -e qsub.log -m a"
```
...
``` {admonition} Non-essential Module
:class: Tip
Databases are automatically setup automatically when running any vomix-snakemake module that requires database. If you would like to install all databases in one go, you may use this module. 
```

``` {admonition} Database Sizes
:class: Warning
