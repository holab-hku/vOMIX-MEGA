...
_1.2 Download the GitHub repository_
...
_1.3 Test Viral Contig Identification using Sample Data_
```bash
...
--cores 
   Use at most N CPU cores/jobs in parallel. If N is omitted or 'all', the limit is set to the number of available CPU cores. In the case of cluster/cloud execution, this argument sets the maximum number of cores requested from the cluster or cloud
...
--config
    ...
    fasta
        Single-sample fasta input for analysis. Available only in certain modules. Read more in our docs. 
    outdir
...
> NOTE: To see all snakemake parameters run `snakemake -h`. Configuration nested after `--config` are all specific to vOMIX-MEGA and can only be accessed via the `config/config.yml` file. Please read more in our [Run & Configuration](https://github.com/holab-hku/vOMIX-MEGA/wiki/Run-&-Configuration) documentation page. 
...
--cores 
   Use at most N CPU cores/jobs in parallel. If N is omitted or 'all', the limit is set to the number of available CPU cores. In case of cluster/cloud execution, this argument sets the maximum number of cores requested from the cluster or cloud
...
--config 
    ...
    decontam-host=False
        Flag to decontaminate host during pre-processing. If set to True it will automatically downloads and uses the human-t2t-hla index
    outdir
...
The pipeline handles all processes. To run on a cluster, you can visit [Advanced Usage](https://github.com/holab-hku/vOMIX-MEGA/wiki/Advanced-Usage) for a detailed setup guide. To change the pipeline configuration, you may change the `config.yml` file or visit [Run & Configuration](https://github.com/holab-hku/vOMIX-MEGA/wiki/Run-&-Configuration) for a detailed run and configuration explanation. 
